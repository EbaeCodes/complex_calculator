...
#define MAX_STACK_DEPTH 10 //declaring a constant

//declaring structure for RPN operation
typedef struct {
...
complex complex_division(complex c1, complex c2) {
	complex result = { 0,0 };
	int squared = ((c2.real * c2.real) + (c2.imag * c2.imag));
	result.real = ((c1.real * c2.real) + (c1.imag * c2.imag)) / squared;
	result.imag = ((c2.real * c1.imag) - (c1.real * c2.imag)) / squared;

	return result;
}
//for squaring complex number
...
//function that determines which method that will implemented depending on the operator
//put by the user
...
complex complex_calc(int argc, char** argv) {
	//declaring complex variables for storing values
	complex result = { 0, 0 };
	complex c;
	complex c1 = { 0, 0 };
